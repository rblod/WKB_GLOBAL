!>@file   wkbcdf.F90
!>@brief Low level Netcdf management
...
CONTAINS
  ...
  SUBROUTINE Read_Ncdf_var2d_Int(varname,file,tabvar)
    ...
    !       
    CHARACTER(*),INTENT(in) :: varname,file
    INTEGER, DIMENSION(:,:), ALLOCATABLE :: tabvar
    !local variables
    INTEGER, DIMENSION(10) :: dimIDS
    INTEGER :: dim1,dim2
    INTEGER :: status,ncid
  ...
  SUBROUTINE Write_Ncdf_var2d_Real_bis(varname,dimname,file,tabvar,nbdim,typevar)
    ...
    REAL(wp), DIMENSION(:,:,:), ALLOCATABLE :: tabtemp3d
    REAL(wp), DIMENSION(:,:,:,:), ALLOCATABLE :: tabtemp4d
    !
    ! local variables
    !
  ...
  SUBROUTINE Read_Ncdf_VarName(filename,tabvarname)
    ...
    CHARACTER*20,DIMENSION(:), ALLOCATABLE :: tabvarname
